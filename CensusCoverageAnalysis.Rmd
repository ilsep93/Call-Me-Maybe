...
pp_df %<>%
  mutate(zeroto17 = ifelse(edad<=17, 1,0),
         eighteento29= ifelse(edad >=18 & edad<=29, 1,0),
         thirtyto44 = ifelse(edad >=30 & edad<=44, 1,0),
         fortyfiveto64 = ifelse(edad>=45 & edad<=64,1,0),
         sixtyfiveup = ifelse(edad>=65,1,0)
         )
...
```{r}
population <- merged %>%
  select(-id_viv, -agecat, -educat, -wealth) %>%
  group_by(ent) %>%
...
educat = case_when(
   nivacad==2 | nivacad==6 ~ 1,
   nivacad==3 | nivacad==7 ~ 2,
   nivacad==4 | nivacad==8 ~ 3,
   nivacad %in% c(0, 1, 5,9,10,11,12) ~ 4),
#Collapse wealth 4 and 5 (low rate for 5)
  wealth = recode(wealth, `5` = "4")
  ) %>%
  #Removing missing observations
  select(ent, celular, educat, agecat, wealth) %>%
  na.omit() %>%
  mutate_at(vars(agecat, educat, wealth), funs(as.factor))
```
...
Cell-phone households in Jalisco (no missing obs)
```{r}
cellonly_jal <- merged_rake %>%
  ungroup () %>%
  filter(celular==1 & ent==14) %>%
  #No obs must be missing
  select(agecat, educat, wealth)
